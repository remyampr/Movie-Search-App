...
let currentPage = 1;
// fro getting all movies contains  "hero"
const searchMovieKey = "hero";

// automatically calls when page load
getFullMovies();
...
async function getFullMovies() {
  //  fetch all movies with action and calldisplayMovieCard with that movies to display on home page
  try {
...
function displayMovieCards(movies) {
  ...
  movies.forEach((element) => {
    // console.log("MOVIESSS : ",element );
    // console.log("**************&&&&&&&&&***********");
    let src = element.Poster;
    let movieCardCol = document.createElement("div");
    movieCardCol.classList.add("col-12", "col-sm-6", "col-md-4", "col-lg-3","movie-card-col");
    ...
    // when clicke on see more set an event listner so it can direct to display that movie detail using function  getSearchedMovie(moviename)
...
// to fetch more movies when window end
